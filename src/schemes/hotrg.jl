...
mutable struct HOTRG <: TNRScheme
    ...
    finalize!::Function
    function HOTRG(T::PFTensor; finalize = (finalize!))
...
function _get_hotrg_xproj(
        A1::PFTensor, A2::PFTensor, trunc::TensorKit.TruncationScheme
    )
...
function _get_hotrg_yproj(
    ...
    _, s′, U′, ε′ = tsvd!(MM; trunc)
    @debug "Uy top truncation" singular_values = s′ trunc_err = ε′
    if ε > ε′
        U, s, ε = adjoint(U′), s′, ε′
    end
    return U, s, ε
