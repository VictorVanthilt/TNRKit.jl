...
mutable struct ImpurityHOTRG <: TNRScheme
    ...
    T_imp_2::TensorMap
    finalize!::Function
    function ImpurityHOTRG(T::TensorMap{E, S, 2, 2}, T_imp_1::TensorMap{E, S, 2, 2}, T_imp_2::TensorMap{E, S, 2, 2}, ; finalize = (finalize!)) where {E, S}
        ...
        @assert space(T, 4) == space(T_imp_1, 4) == space(T_imp_2, 4) "Fourth index space of T, T_imp_1 and T_imp_2 must be the same"
        return new(T, T_imp_1, T_imp_2, finalize)
    end
end
...
function step!(scheme::HOTRG, trunc::TensorKit.TruncationScheme)
    ...
    @tensor scheme.T_imp1[-1 -2; -3 -4] := 1/2 * scheme.T_imp_1[1 5; -3 3] * conj(U[1 2; -1]) * U[3 4; -4] *
        scheme.T[2 -2; 5 4] + 1/2 * scheme.T[1 5; -3 3] * conj(U[1 2; -1]) * U[3 4; -4] *
        scheme.T_imp_1[2 -2; 5 4]

    @tensor scheme.T_imp2[-1 -2; -3 -4] := 1/4* scheme.T_imp_2[1 5; -3 3] * conj(U[1 2; -1]) * U[3 4; -4] *
        scheme.T[2 -2; 5 4] + 1/4 * scheme.T[1 5; -3 3] * conj(U[1 2; -1]) * U[3 4; -4] *
        scheme.T_imp_2[2 -2; 5 4] + 1/2 * scheme.T_imp_1[1 5; -3 3] * conj(U[1 2; -1]) * U[3 4; -4] *
        scheme.T_imp_1[2 -2; 5 4]
    ...
    @tensor scheme.T_imp1[-1 -2; -3 -4] := 1/2 * scheme.T_imp1[-1 1; 3 5] * scheme.T[5 2; 4 -4] *
        conj(U[1 2; -2]) * U[3 4; -3] + 1/2 * scheme.T[-1 1; 3 5] * scheme.T_imp1[5 2; 4 -4] *
        conj(U[1 2; -2]) * U[3 4; -3]

    @tensor scheme.T_imp2[-1 -2; -3 -4] := 1/4 * scheme.T_imp2[-1 1; 3 5] * scheme.T[5 2; 4 -4] *
        conj(U[1 2; -2]) * U[3 4; -3] + 1/4 * scheme.T[-1 1; 3 5] * scheme.T_imp2[5 2; 4 -4] *
        conj(U[1 2; -2]) * U[3 4; -3] + 1/2 * scheme.T_imp1[-1 1; 3 5] * scheme.T_imp1[5 2; 4 -4] *
        conj(U[1 2; -2]) * U[3 4; -3]
...
function Base.show(io::IO, scheme::ImpurityHOTRG)
    ...
    println(io, "  * T_imp_1: $(summary(scheme.T_imp_1))")
    println(io, "  * T_imp_2: $(summary(scheme.T_imp_2))")
    return nothing
end
