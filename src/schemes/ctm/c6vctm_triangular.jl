...
### Running the algorithm
    run!(::c6vCTM, trunc::TruncationStrategy, stop::Stopcrit[, finalize_beginning=true, projectors=:twothirds, conditioning=true, verbosity=1])
...
function calculate_twothirds_projectors(scheme::c6vCTM_triangular, trunc)
    ...
    U, S, V = svd_trunc(ρρ; trunc = trunc & truncbelow(1.0e-20), alg = TensorKit.SVD())
...
function calculate_full_projectors(scheme::c6vCTM_triangular, trunc)
    ρ = build_double_corner_matrix_triangular(scheme)
    ρ /= norm(ρ)
    Ū, S̄, V̄ᴴ = svd_trunc(ρ; trunc = truncbelow(1.0e-20), alg = TensorKit.SVD())
    ρ̄ᴿ = Ū * sqrt(S̄)
    ρ̄ᴸ = sqrt(S̄) * V̄ᴴ
    @tensor ρρ[-1; -2] := ρ̄ᴸ[-1; 1 2] * flip(ρ, 2; inv = false)[1 2; 3 4] * flip(ρ, 2; inv = false)[3 4; 5 6] * flip(ρ̄ᴿ, 2; inv = false)[5 6; -2]
    ρρ /= norm(ρρ)

    U, S, Vᴴ = svd_trunc(ρρ; trunc = trunc & truncbelow(1.0e-20), alg = TensorKit.SVD())
...
function semi_renormalize(scheme::c6vCTM_triangular, Pa, Pb, trunc)
    ...
    U, S, V = svd_trunc(mat; trunc = truncbelow(1.0e-20), alg = TensorKit.SVD())
    Ẽb = U * sqrt(S)
    Ẽa = permute(sqrt(S) * V, ((1, 3), (2,)))

    Utr, Str, Vtr = svd_trunc(mat; trunc = trunc & truncbelow(1.0e-20), alg = TensorKit.SVD())
    Ẽbtr = Utr * sqrt(Str)
...
function build_matrix_second_projector(scheme::c6vCTM_triangular, Ẽa, Ẽb, Ẽatr, Ẽbtr; conditioning = true)
    ...
    if conditioning
        σL /= norm(σL)
        σR /= norm(σR)
        UL, SL, VLᴴ = svd_trunc(σL; trunc = truncbelow(1.0e-20), alg = TensorKit.SVD())
        UR, SR, VRᴴ = svd_trunc(σR; trunc = truncbelow(1.0e-20), alg = TensorKit.SVD())
        ...
        mat = FLU * FRU
        mat /= norm(mat)
        WU, SU, QUᴴ = svd_trunc(mat; trunc = truncbelow(1.0e-20), alg = TensorKit.SVD())
