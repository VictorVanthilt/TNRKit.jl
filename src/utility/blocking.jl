function block_tensors(O_list::Matrix)
    ...
    Vv = [space(O_list[end, j])[2] for j = 1:n]
    Uv = isomorphism(fuse(Vv...), prod(Vv))
    Vh = [space(O_list[i, 1])[1] for i = 1:m]
    Uh = isomorphism(fuse(Vh...), prod(Vh))

    ind1 = vcat(-1, [(n + 1) * (i - 1) + 1 for i = 1:m])
    ind2 = vcat(-2, [m * (n + 1) + (m + 1) * j for j = 1:n])
    ind3 = vcat([m * (n + 1) + (m + 1) * (j - 1) + 1 for j = 1:n], -3)
    ind4 = vcat([(n + 1) * i for i = 1:m], -4)
