...
function QR_L(L::TensorMap, T::AbstractTensorMap{E,S,M,N}, in_ind, out_ind) where {E,S, M, N}
    permT = ((in_ind,), (reverse(collect(1:in_ind - 1))...,collect(M+1:M+N)..., reverse(collect(in_ind + 1:M))...))
    permLT = ((reverse(collect(2:in_ind+out_ind-1))..., 1, reverse(collect(in_ind+out_ind+1:M+N))...), (in_ind+out_ind,))
...
function QR_R(R::TensorMap, T::AbstractTensorMap{E,S,M,N}, in_ind, out_ind) where {E,S, M, N}
    permT = ((reverse(collect(M+1:M+in_ind-1))..., collect(1:M)..., reverse(collect(M+in_ind+1:M+N))...), (M+in_ind,))
    permTR = ((in_ind+out_ind-1,), (reverse(collect(1:in_ind+out_ind-2))..., M+N, reverse(collect(in_ind+out_ind:M+N-1))...))
...
function find_L(psi::Array, in_inds::Array, out_inds::Array, entanglement_criterion::stopcrit)
    ...
    while crit
        ...
        L_list[pos_next] = QR_L(L_list[running_pos], psi[running_pos], in_inds[running_pos], out_inds[running_pos])
...
function find_R(psi::Array, in_inds::Array, out_inds::Array, entanglement_criterion::stopcrit)
    ...
    while crit
        ...
        R_list[pos_last] = QR_R(R_list[running_pos], psi[running_pos], in_inds[running_pos], out_inds[running_pos])
...
function find_projectors(psi::Array, in_inds::Array, out_inds::Array, entanglement_criterion::stopcrit,
...
function MPO_disentangled!(psi::Array, in_inds::Array, out_inds::Array, PR_list::Array, PL_list::Array)
    ...
    for i in 1:n
        ...
        permT = ((in_ind,), (reverse(collect(1:in_ind - 1))...,collect(M+1:M+N)..., reverse(collect(in_ind + 1:M))...))
        permLT = ((reverse(collect(2:in_ind+out_ind-1))..., 1, reverse(collect(in_ind+out_ind+1:M+N))...), (in_ind+out_ind,))
        permLTR = (Tuple(collect(out_ind:out_ind+M-1)), (collect(reverse(1:out_ind-1))..., M+N, collect(reverse(out_ind+M:M+N-1))...))
        LTR = transpose(transpose(PL_list[i] * transpose(psi[i], permT), permLT) * PR_list[mod(i, n) + 1], permLTR)
        psi[i] = LTR
    end
end
