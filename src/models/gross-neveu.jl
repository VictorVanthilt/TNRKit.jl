function P_tensor()
    ...
    for (pi1, pj1, pi2, pj2, i1, j1, i2, j2) in Iterators.product([0:1 for _ = 1:8]...)
...
function gross_neveu_8_leg_tensor(μ::Number, m::Number, g::Number)
    ...
    for (pi1, pj1, pi2, pj2, i1, j1, i2, j2) in Iterators.product([0:1 for _ = 1:8]...)
        ...
        T[pi1+1, pj1+1, pi2+1, pj2+1, i2+1, j2+1, i1+1, j1+1] =
            ((-1)^p) *
            exp(0.5 * μ * (i2 - j2 + pi2 - pj2)) *
            ((1 / sqrt(2))^(i1 + i2 + j1 + j2 + pi1 + pi2 + pj1 + pj2)) *
            (
                ((m + 2)^2 + 2 * g^2) *
                δ(i1 + i2 + pj1 + pj2, 0) *
                δ(j1 + j2 + pi1 + pi2, 0) -
...
function gross_neveu_start(μ::Number, m::Number, g::Number)
    ...
    @tensor T_fused[-1 -2; -3 -4] :=
        T_unfused[1 2 3 4; 5 6 7 8] * U[-1; 1 2] * U[-2; 3 4] * Udg[5 6; -3] * Udg[7 8; -4]
